...
/** Named checkpoints derived from POINTS — single source of truth. */
export const LANDMARKS: Landmark[] = POINTS
  .filter((p): p is typeof p & { steps: number; name: string } =>
    p.steps !== undefined && p.name !== undefined)
  .map((p) => ({ name: p.name, steps: p.steps, x: p.x, y: p.y }));
...
/** Default route — the detailed POINTS path with Catmull-Rom smoothing. */
